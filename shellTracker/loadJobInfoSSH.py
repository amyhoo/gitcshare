...
import re
import json
from ControlMJobsToLogMapper.tools.SSH import *
from collections import OrderedDict
from functools import cmp_to_key
from ControlMJobsToLogMapper.tools.topological import toposort2
class JobInfo:
    '''
    combine the three table ,and output the information of  job into the ControlMOoutput.csv
    '''        
    def outputInfo(self):      
        header1, jobList = self.loadJobIndex(filename='H:\projects\Devops task\ControlMNow.csv')
        header2, jobBuffer = self.loadJobIndex(filename='H:\projects\Devops task\ControlMJobRef.csv')
        header3, paramBuffer = self.loadJobParams(filename='H:\projects\Devops task\ControlMJobParam.csv')
        with open('H:\projects\Devops task\ControlMOoutput.csv', 'w+') as output:
...
class JobAnalysis():
    ...
    def analysisCmd(self):
        ...
        '''
        buffer={}
        with open('H:\projects\Devops task\ControlMOutput.csv', 'r') as inFiler,open('H:\projects\Devops task\ControlMInfo.csv','w+') as outFiler:
            ...
            if m:
                ...
                for item in m1:
                    ...
                    if item_key in var_dict:
                        continue
                    if re.match('\d+',item_key):#è¿™ç§�å�˜é‡�çœ�ç•¥æŸ¥è¯¢                        
                        try:
        ...
        while index<len(var_dict):            
            ...
            if key in set_var:
                ...
                for var in re.findall(r'`.+`', value):
                    stdin, stdout, stderr=client.exec_command(var.strip("`"))
                    out=stdout.read()
                    err=stderr.read()
                    if not err and out:
                        out=out.decode('utf8')
    ...
    def testLog(self,client,var_dict):
        ...
        for key in test_key:
            if key in var_dict:
                value=var_dict[key]
                stdin, stdout, stderr=client.exec_command('ls '+value)
                out=stdout.read()
                err=stderr.read()
                if not err and out:       
                    result[key]='exist'
    ...
    def remoteScriptInfo(self):
        ...
        '''        
        with SSHClient('Nzarp2', 22, 'u391812', 'Xumin&06') as client:  
            while True:
                filename,cmd = yield
                stdin, stdout, stderr = client.exec_command('ls ' + filename)
                ...
                if stderr.read():
                    ...
                    info = {}
                    with SFTPSession(client) as session, SSHFile(session,filename) as remote_file:    
